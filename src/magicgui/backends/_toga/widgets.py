"""Widget implementations (adaptors) for the Toga backend."""
...
class TogaBaseWidget(protocols.WidgetProtocol):
    """Implements show/hide/native."""

    _toga_widget: toga.Widget

    def __init__(
        self, qwidg: type[toga.Widget], parent: toga.Widget | None = None, **kwargs: Any
    ) -> None:
        self._toga_widget = qwidg(parent=parent)

    def _mgui_close_widget(self) -> None:
        self._toga_widget.close()

    def _mgui_get_visible(self) -> bool:
        return self._toga_widget.isVisible()

    def _mgui_set_visible(self, value: bool) -> None:
        self._toga_widget.setVisible(value)

    def _mgui_get_enabled(self) -> bool:
        return self._toga_widget.isEnabled()

    def _mgui_set_enabled(self, enabled: bool) -> None:
        self._toga_widget.setEnabled(enabled)

    def _mgui_get_parent(self) -> Widget | None:
        par = self._toga_widget.parent
    ...
    def _mgui_render(self) -> numpy.ndarray:
        ...
        img = self._toga_widget.grab().toImage()
        bits = img.constBits()
...
class TogaBaseValueWidget(TogaBaseWidget, protocols.ValueWidgetProtocol):
    """Implements get/set/bind_change."""

    def __init__(
        self,
        qwidg: type[toga.Widget],
        getter: str,
        setter: str,
        onchange: str,
        **kwargs: Any,
    ...
    def _mgui_get_value(self) -> Any:
        val = getattr(self._toga_widget, self._getter_name)()
        return self._post_get_hook(val)

    def _mgui_set_value(self, value: Any) -> None:
        val = self._pre_set_hook(value)
        getattr(self._toga_widget, self._setter_name)(val)

    def _mgui_bind_change_callback(self, callback: Callable) -> None:
        signal_instance = getattr(self._toga_widget, self._onchange_name, None)
        if signal_instance:
...
class EmptyWidget(TogaBaseWidget):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(toga.Widget, **kwargs)
...
class QBaseStringWidget(TogaBaseValueWidget):
    _toga_widget: QtW.QLineEdit | QtW.QTextEdit | QtW.QLabel
...
class Label(QBaseStringWidget):
    _toga_widget: QtW.QLabel

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QLabel, "text", "setText", "", **kwargs)
        self._toga_widget.setSizePolicy(
...
class Image(TogaBaseValueWidget):
    _toga_widget: _ResizeableLabel
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._toga_widget.resized.connect(self._rescale)
        self._pixmap: QPixmap | None = None

    def _rescale(self) -> None:
        if self._pixmap:
            sz = self._toga_widget.size()
            self._toga_widget.setPixmap(
                self._pixmap.scaled(
                    sz,
...
class QuantityEdit(TogaBaseValueWidget):
    _toga_widget: superqt.QQuantity
...
class Password(LineEdit):
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._toga_widget.setEchoMode(QtW.QLineEdit.EchoMode.Password)
...
class TextEdit(QBaseStringWidget, protocols.SupportsReadOnly):
    ...
    def _mgui_set_read_only(self, value: bool) -> None:
        self._toga_widget.setReadOnly(value)

    def _mgui_get_read_only(self) -> bool:
        return self._toga_widget.isReadOnly()


# NUMBERS


class QBaseRangedWidget(TogaBaseValueWidget, protocols.RangedWidgetProtocol):
    """Provides min/max/step implementations."""

    _toga_widget: QtW.QDoubleSpinBox | superqt.QLargeIntSpinBox | QtW.QAbstractSlider
    _precision: float = 1

    def __init__(self, qwidg: type[toga.Widget], **kwargs: Any) -> None:
        super().__init__(qwidg, "value", "setValue", "valueChanged", **kwargs)

    def _mgui_get_min(self) -> float:
        """Get the minimum possible value."""
        val = self._toga_widget.minimum()
        return self._post_get_hook(val)
    ...
    def _mgui_set_min(self, value: float):
        ...
        val = self._pre_set_hook(value)
        self._toga_widget.setMinimum(val)

    def _mgui_get_max(self) -> float:
        """Set the maximum possible value."""
        val = self._toga_widget.maximum()
        return self._post_get_hook(val)
    ...
    def _mgui_set_step(self, value: float):
        ...
        val = self._pre_set_hook(value)
        self._toga_widget.setSingleStep(val)
...
class QBaseButtonWidget(
    TogaBaseValueWidget, protocols.SupportsText, protocols.SupportsIcon
):
    _toga_widget: QtW.QCheckBox | QtW.QPushButton | QtW.QRadioButton | QtW.QToolButton
    ...
    def _mgui_set_text(self, value: str) -> None:
        """Set text."""
        self._toga_widget.setText(str(value))

    def _mgui_get_text(self) -> str:
        """Get text."""
        return self._toga_widget.text()

    def _update_icon(self) -> None:
        # Called when palette changes or icon is set
        if self._icon:
            qicon = _get_qicon(*self._icon, palette=self._toga_widget.palette())
            if qicon is None:
                self._icon = None  # an error occurred don't try again
                self._toga_widget.setIcon(QIcon())
            else:
                self._toga_widget.setIcon(qicon)
...
class PushButton(QBaseButtonWidget):
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._toga_widget.setAutoDefault(True)
...
class Container(
    TogaBaseWidget, protocols.ContainerProtocol, protocols.SupportsOrientation
):
    def __init__(
        self, layout="vertical", scrollable: bool = False, **kwargs: Any
    ) -> None:
        TogaBaseWidget.__init__(self, toga.Widget, **kwargs)
        if layout == "horizontal":
            self._layout: QtW.QBoxLayout = QtW.QHBoxLayout()
        else:
            self._layout = QtW.QVBoxLayout()
        self._toga_widget.setLayout(self._layout)
        ...
        if scrollable:
            ...
            self._scroll.setWidget(self._toga_widget)
            self._toga_widget = self._scroll

    @property
    def _is_scrollable(self) -> bool:
        return isinstance(self._toga_widget, QtW.QScrollArea)
    ...
    def _mgui_get_native_widget(self):
        ...
        return self._toga_widget.widget() if self._is_scrollable else self._toga_widget
...
class MainWindow(Container):
    def __init__(
        ...
        if scrollable:
            ...
            self._main_window.setCentralWidget(self._toga_widget)
...
class FloatSpinBox(QBaseRangedWidget):
    ...
    def _mgui_set_step(self, value: float):
        """Set the step size."""
        if value and value < 1 * 10 ** -self._toga_widget.decimals():
            self._toga_widget.setDecimals(math.ceil(abs(math.log10(value))))
        self._toga_widget.setSingleStep(value)


class _Slider(QBaseRangedWidget, protocols.SupportsOrientation):
    _toga_widget: QtW.QSlider
    ...
    def _mgui_set_orientation(self, value) -> Any:
        ...
        self._toga_widget.setOrientation(orientation)

    def _mgui_get_orientation(self) -> Any:
        """Get current value of the widget."""
        orientation = self._toga_widget.orientation()
    ...
    def _mgui_get_tracking(self) -> bool:
        ...
        if hasattr(self._toga_widget, "hasTracking"):
            return self._toga_widget.hasTracking()
        return False

    def _mgui_set_tracking(self, value: bool) -> None:
        # Progressbar also uses this base, but doesn't have tracking
        if hasattr(self._toga_widget, "setTracking"):
            self._toga_widget.setTracking(value)
...
class Slider(_Slider):
    ...
    def __init__(self, qwidg=QtW.QSlider, **kwargs: Any) -> None:
        ...
        self._toga_widget.valueChanged.connect(self._on_slider_change)
    ...
    def _mgui_set_orientation(self, value: str) -> None:
        ...
        if value == "vertical":
            ...
            self._toga_widget.setOrientation(Qt.Vertical)
            layout.addWidget(self._toga_widget, alignment=Qt.AlignmentFlag.AlignHCenter)
            layout.addWidget(
            ...
            self._toga_widget.setOrientation(Qt.Horizontal)
            layout.addWidget(self._toga_widget)
        ...
        if old_layout is not None:
            toga.Widget().setLayout(old_layout)
    ...
    def _on_readout_change(self):
        self._toga_widget.setValue(self._pre_set_hook(self._readout_widget.value()))
...
class FloatSlider(Slider):
    ...
    def _mgui_bind_change_callback(self, callback):
        ...
        self._toga_widget.valueChanged.connect(_converted_value)


class RangeSlider(_Slider):
    _toga_widget: superqt.QLabeledRangeSlider

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(superqt.QLabeledRangeSlider, **kwargs)
        if hasattr(self._toga_widget, "applyMacStylePatch"):
            # >= magicgui v0.5.2
            self._toga_widget.applyMacStylePatch()

    def _mgui_set_readout_visibility(self, value: bool):
        method = "show" if value else "hide"
        try:
            for label in chain(
                self._toga_widget._handle_labels,
                [self._toga_widget._min_label, self._toga_widget._max_label],
            ):
...
class FloatRangeSlider(RangeSlider):
    _toga_widget: superqt.QLabeledDoubleRangeSlider
...
class ProgressBar(_Slider):
    ...
    def _mgui_set_readout_visibility(self, value: bool):
        self._toga_widget.setTextVisible(value)


class ComboBox(TogaBaseValueWidget, protocols.CategoricalWidgetProtocol):
    _toga_widget: QtW.QComboBox

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QComboBox, "isChecked", "setCurrentIndex", "", **kwargs)
        self._toga_widget.currentIndexChanged.connect(self._emit_data)

    def _emit_data(self, index: int):
        self._event_filter.valueChanged.emit(self._toga_widget.itemData(index))
    ...
    def _mgui_get_count(self) -> int:
        ...
        return self._toga_widget.count()

    def _mgui_get_choice(self, choice_name: str) -> Any:
        item_index = self._toga_widget.findText(choice_name)
        return None if item_index == -1 else self._toga_widget.itemData(item_index)

    def _mgui_get_current_choice(self) -> str:
        return self._toga_widget.itemText(self._toga_widget.currentIndex())

    def _mgui_get_value(self) -> Any:
        return self._toga_widget.itemData(self._toga_widget.currentIndex())

    def _mgui_set_value(self, value) -> None:
        # Note: there's a bug in PyQt6, where CombBox.findData(value) will not
        # find the data if value is an Enum. So we do it manually
        wdg = self._toga_widget
        idx = next((i for i in range(wdg.count()) if wdg.itemData(i) == value), -1)
        self._toga_widget.setCurrentIndex(idx)
    ...
    def _mgui_set_choice(self, choice_name: str, data: Any) -> None:
        ...
        if item_index == -1:
            self._toga_widget.addItem(choice_name, data)
        # otherwise update its data
        else:
            self._toga_widget.setItemData(item_index, data)
    ...
    def _mgui_set_choices(self, choices: Iterable[tuple[str, Any]]) -> None:
        ...
        if not choices_:
            self._toga_widget.clear()
            return

        with _signals_blocked(self._toga_widget):
            choice_names = [x[0] for x in choices_]
            # remove choices that no longer exist
            current = self._toga_widget.itemText(self._toga_widget.currentIndex())
            for i in reversed(range(self._toga_widget.count())):
                if self._toga_widget.itemText(i) not in choice_names:
                    self._toga_widget.removeItem(i)
            # update choices
            ...
            current2 = self._toga_widget.itemText(self._toga_widget.currentIndex())
            if current not in choice_names:
                # previous value was not in the new choices so set first element
                first = choice_names[0]
                self._toga_widget.setCurrentIndex(self._toga_widget.findText(first))
            elif current2 != current:
                # element is present but order is different
                self._toga_widget.setCurrentIndex(self._toga_widget.findText(current))
        if current not in choice_names:
            self._emit_data(self._toga_widget.currentIndex())

    def _mgui_del_choice(self, choice_name: str) -> None:
        """Delete choice_name."""
        item_index = self._toga_widget.findText(choice_name)
        if item_index >= 0:
            self._toga_widget.removeItem(item_index)

    def _mgui_get_choices(self) -> tuple[tuple[str, Any], ...]:
        """Get available choices."""
        return tuple(
            (self._toga_widget.itemText(i), self._toga_widget.itemData(i))
            for i in range(self._toga_widget.count())
        )


class Select(TogaBaseValueWidget, protocols.CategoricalWidgetProtocol):
    _toga_widget: QtW.QListWidget

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QListWidget, "isChecked", "setCurrentIndex", "", **kwargs)
        self._toga_widget.itemSelectionChanged.connect(self._emit_data)
        self._toga_widget.setSelectionMode(
            QtW.QAbstractItemView.SelectionMode.ExtendedSelection
        )

    def _emit_data(self):
        data = self._toga_widget.selectedItems()
    ...
    def _mgui_get_choice(self, choice_name: str) -> list[Any]:
        items = self._toga_widget.findItems(choice_name, Qt.MatchExactly)
        return [i.data(Qt.ItemDataRole.UserRole) for i in items]

    def _mgui_get_current_choice(self) -> list[str]:  # type: ignore[override]
        return [i.text() for i in self._toga_widget.selectedItems()]

    def _mgui_get_value(self) -> Any:
        return [
            i.data(Qt.ItemDataRole.UserRole) for i in self._toga_widget.selectedItems()
        ]

    def _mgui_set_value(self, value) -> None:
        if not isinstance(value, (list, tuple)):
            value = [value]
        selected_prev = self._toga_widget.selectedItems()
        with _signals_blocked(self._toga_widget):
            for i in range(self._toga_widget.count()):
                item = self._toga_widget.item(i)
                item.setSelected(item.data(Qt.ItemDataRole.UserRole) in value)
        selected_post = self._toga_widget.selectedItems()
        if selected_prev != selected_post:
            self._emit_data()

    def _mgui_set_choice(self, choice_name: str, data: Any) -> None:
        """Set data for ``choice_name``."""
        items = self._toga_widget.findItems(choice_name, Qt.MatchFlag.MatchExactly)
        # if it's not in the list, add a new item
        if not items:
            item = QtW.QListWidgetItem(choice_name)
            item.setData(Qt.ItemDataRole.UserRole, data)
            self._toga_widget.addItem(item)
        # otherwise update its data
        else:
            for item in items:
                item.setData(Qt.ItemDataRole.UserRole, data)
    ...
    def _mgui_set_choices(self, choices: Iterable[tuple[str, Any]]) -> None:
        ...
        with _signals_blocked(self._toga_widget):
            ...
            # remove choices that no longer exist
            for i in reversed(range(self._toga_widget.count())):
                if self._toga_widget.item(i).text() not in choice_names:
                    self._toga_widget.takeItem(i)
            # update choices
            for name, data in choices_:
                self._mgui_set_choice(name, data)
            selected_post = self._toga_widget.selectedItems()
    ...
    def _mgui_del_choice(self, choice_name: str) -> None:
        """Delete choice_name."""
        for i in reversed(range(self._toga_widget.count())):
            if self._toga_widget.item(i).text() == choice_name:
                self._toga_widget.takeItem(i)
    ...
    def _mgui_get_choices(self) -> tuple[tuple[str, Any], ...]:
        ...
        return tuple(
            (
                self._toga_widget.item(i).text(),
                self._toga_widget.item(i).data(Qt.ItemDataRole.UserRole),
            )
            for i in range(self._toga_widget.count())
        )
...
class RadioButtons(
    ...
    _toga_widget: QtW.QGroupBox

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QGroupBox, "", "", "", **kwargs)
        self._btn_group = QtW.QButtonGroup(self._toga_widget)
    ...
    def _mgui_set_orientation(self, value: str) -> None:
        ...
        old_layout = self._toga_widget.layout()
        if old_layout is not None:
            toga.Widget().setLayout(old_layout)
        self._toga_widget.setLayout(new_layout)
        self._toga_widget.layout().setContentsMargins(4, 4, 4, 4)

    def _mgui_get_orientation(self) -> str:
        """Get orientation, return either 'horizontal' or 'vertical'."""
        if isinstance(self._toga_widget.layout(), QtW.QVBoxLayout):
            return "vertical"
        else:
            return "horizontal"
    ...
    def _add_button(self, label: str, data: Any | None = None):
        btn = QtW.QRadioButton(label, self._toga_widget)
        btn._data = data
        self._btn_group.addButton(btn)
        self._toga_widget.layout().addWidget(btn)
...
class DateTimeEdit(TogaBaseValueWidget):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(
    ...
    def _mgui_get_value(self):
        try:
            return self._toga_widget.dateTime().toPython()
        except (TypeError, AttributeError):
            return self._toga_widget.dateTime().toPyDateTime()


class DateEdit(TogaBaseValueWidget):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QDateEdit, "", "setDate", "dateChanged", **kwargs)

    def _mgui_get_value(self):
        try:
            return self._toga_widget.date().toPython()
        except (TypeError, AttributeError):
            return self._toga_widget.date().toPyDate()


class TimeEdit(TogaBaseValueWidget):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QTimeEdit, "", "setTime", "timeChanged", **kwargs)

    def _mgui_get_value(self):
        try:
            return self._toga_widget.time().toPython()
        except (TypeError, AttributeError):
            return self._toga_widget.time().toPyTime()


class ToolBar(TogaBaseWidget):
    _toga_widget: QtW.QToolBar

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(QtW.QToolBar, **kwargs)
        self._toga_widget.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
        self._event_filter.paletteChanged.connect(self._on_palette_change)

    def _on_palette_change(self):
        for action in self._toga_widget.actions():
            if icon := action.data():
                if qicon := _get_qicon(icon, None, palette=self._toga_widget.palette()):
                    action.setIcon(qicon)

    def _mgui_add_button(self, text: str, icon: str, callback: Callable) -> None:
        """Add an action to the toolbar."""
        act = self._toga_widget.addAction(text, callback)
        if qicon := _get_qicon(icon, None, palette=self._toga_widget.palette()):
            act.setIcon(qicon)
            act.setData(icon)
    ...
    def _mgui_add_separator(self) -> None:
        ...
        self._toga_widget.addSeparator()
    ...
    def _mgui_set_icon_size(self, size: int | tuple[int, int] | None) -> None:
        ...
        if isinstance(size, int):
            ...
            _size = QSize()
        self._toga_widget.setIconSize(_size)
...
class Dialog(TogaBaseWidget, protocols.ContainerProtocol):
    def __init__(
        self, layout="vertical", scrollable: bool = False, **kwargs: Any
    ) -> None:
        TogaBaseWidget.__init__(self, QtW.QDialog, **kwargs)
        if layout == "horizontal":
            self._layout: QtW.QBoxLayout = QtW.QHBoxLayout()
        else:
            self._layout = QtW.QVBoxLayout()
        ...
        button_box.accepted.connect(self._toga_widget.accept)
        button_box.rejected.connect(self._toga_widget.reject)
    ...
    def _mgui_exec(self) -> Any:
        return self._toga_widget.exec_()
...
class Table(TogaBaseWidget, protocols.TableWidgetProtocol):
    _toga_widget: _QTableExtended

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(_QTableExtended, **kwargs)
        header = self._toga_widget.horizontalHeader()
        # avoid strange AttributeError on CI
        if hasattr(header, "setSectionResizeMode"):
            header.setSectionResizeMode(QtW.QHeaderView.ResizeMode.Stretch)
        # self._toga_widget.horizontalHeader().setSectionsMovable(True)  # tricky!!
        header.setSectionResizeMode(QtW.QHeaderView.ResizeMode.Interactive)
        self._toga_widget.itemChanged.connect(self._update_item_data_with_text)

    def _mgui_set_read_only(self, value: bool) -> None:
        value = bool(value)
        self._toga_widget._read_only = value
        if value:
            self._toga_widget.setEditTriggers(_READ_ONLY)
        else:
            self._toga_widget.setEditTriggers(_EDITABLE)

    def _mgui_get_read_only(self) -> bool:
        return self._toga_widget._read_only
    ...
    def _mgui_set_cell(self, row: int, col: int, value: Any) -> None:
        ...
        if value is None:
            self._toga_widget.setItem(row, col, None)
            self._toga_widget.removeCellWidget(row, col)
            return
        if isinstance(value, Widget):
            self._toga_widget.setCellWidget(row, col, value.native)
            return
        item = QtW.QTableWidgetItem(str(value))
        item.setData(_DATA_ROLE, value)
        self._toga_widget.setItem(row, col, item)
    ...
    def _mgui_get_row_headers(self) -> tuple:
        ...
        indices = range(self._toga_widget.rowCount())
        items = (self._toga_widget.verticalHeaderItem(i) for i in indices)
    ...
    def _mgui_set_row_headers(self, headers: Sequence) -> None:
        ...
        self._toga_widget.setVerticalHeaderLabels(tuple(map(str, headers)))

    def _mgui_get_column_headers(self) -> tuple:
        """Get current column headers of the widget."""
        horiz_header = self._toga_widget.horizontalHeader()
        ncols = self._toga_widget.columnCount()
        ...
        items = (self._toga_widget.horizontalHeaderItem(i) for i in indices)
        headers = (item.text() for item in items if item)
        return tuple(_maybefloat(x) for x in headers)

    def _mgui_set_column_headers(self, headers: Sequence) -> None:
        """Set current column headers of the widget."""
        self._toga_widget.setHorizontalHeaderLabels(tuple(map(str, headers)))
    ...
    def _mgui_bind_change_callback(self, callback):
        ...
        self._toga_widget.itemChanged.connect(partial(self._item_callback, callback))
